...
namespace Print.Message.Dal
{
    class PrintMessageDAL
    {
        ...
        //检查SIM号是否与IMEI绑定，存在返回1，否则返回0
        public int CheckSIMbudingIMEIDAL(string SIM,string IMEI)
        {
            using (SqlConnection conn1 = new SqlConnection(conStr))
            {
                conn1.Open();
                using (SqlCommand command = conn1.CreateCommand())
                {
                    command.CommandText = "SELECT IMEI ,SIM FROM dbo.Gps_ManuPrintParam WHERE SIM='" + SIM + "'";
                    SqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
                    {
                        if(dr.GetString(0) == IMEI)
                            return 0;
                        else if (dr.GetString(0) == "" && dr.GetString(1) == "")
                            return 0;
                        else
                            return 1;
                    }
        ...
        //检查VIP号是否与IMEI绑定，存在返回1，否则返回0
        public int CheckVIPbudingIMEIDAL(string VIP, string IMEI)
        {
            using (SqlConnection conn1 = new SqlConnection(conStr))
            {
                conn1.Open();
                using (SqlCommand command = conn1.CreateCommand())
                {
                    command.CommandText = "SELECT IMEI ,VIP FROM dbo.Gps_ManuPrintParam WHERE VIP='" + VIP + "'";
                    SqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
                    {
                        if (dr.GetString(0) == IMEI)
        ...
        //检查BAT号是否与IMEI绑定，存在返回1，否则返回0
        public int CheckBATbudingIMEIDAL(string BAT, string IMEI)
        {
            using (SqlConnection conn1 = new SqlConnection(conStr))
            {
                conn1.Open();
                using (SqlCommand command = conn1.CreateCommand())
                {
                    command.CommandText = "SELECT IMEI ,BAT FROM dbo.Gps_ManuPrintParam WHERE BAT='" + BAT + "'";
                    SqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
                    {
                        if (dr.GetString(0) == IMEI)
        ...
        //检查ICCID号是否与IMEI绑定，存在返回1，否则返回0
        public int CheckICCIDbudingIMEIDAL(string ICCID, string IMEI)
        {
            using (SqlConnection conn1 = new SqlConnection(conStr))
            {
                conn1.Open();
                using (SqlCommand command = conn1.CreateCommand())
                {
                    command.CommandText = "SELECT IMEI ,ICCID FROM dbo.Gps_ManuPrintParam WHERE ICCID='" + ICCID + "'";
                    SqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
                    {
                        if (dr.GetString(0) == IMEI)
        ...
        //检查MAC号是否与IMEI绑定，存在返回1，否则返回0
        public int CheckMACbudingIMEIDAL(string MAC, string IMEI)
        {
            using (SqlConnection conn1 = new SqlConnection(conStr))
            {
                conn1.Open();
                using (SqlCommand command = conn1.CreateCommand())
                {
                    command.CommandText = "SELECT IMEI ,MAC FROM dbo.Gps_ManuPrintParam WHERE MAC='" + MAC + "'";
                    SqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
                    {
                        if (dr.GetString(0) == IMEI)
        ...
        //检查Equipment号是否与IMEI绑定，存在返回1，否则返回0
        public int CheckEquipmentbudingIMEIDAL(string Equipment, string IMEI)
        {
            using (SqlConnection conn1 = new SqlConnection(conStr))
            {
                conn1.Open();
                using (SqlCommand command = conn1.CreateCommand())
                {
                    command.CommandText = "SELECT IMEI ,Equipment FROM dbo.Gps_ManuPrintParam WHERE Equipment='" + Equipment + "'";
                    SqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
                    {
                        if (dr.GetString(0) == IMEI)
        ...
        //检查RFID号是否与IMEI绑定，存在返回1，否则返回0
        public int CheckRFIDbudingIMEIDAL(string RFID, string IMEI)
        {
            using (SqlConnection conn1 = new SqlConnection(conStr))
            {
                conn1.Open();
                using (SqlCommand command = conn1.CreateCommand())
                {
                    command.CommandText = "SELECT IMEI,RFID FROM dbo.Gps_ManuPrintParam WHERE RFID='" + RFID + "'";
                    SqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
                    {
                        if (dr.GetString(0) == IMEI)
