    ...
    class FuselagePrintRecordParamDAL
    {
      private static  string conStr = ConfigurationManager.ConnectionStrings["conn1"].ConnectionString;
        ...
        public int InsertFuselagePrintRecordParamDAL(string Zhidan, int PrintOneByOneMark, int PltplotMark, int CustomerSupplySNMark, int NoPrintCheckCodeMark, int NoPrintingSNMark, int IMEIHexadecimalMark, int SNHexadecimalMark, int ReplayOneByOneMark, int BattingInBatchesMark, int NoParityBitMark, int HexadecimalMark,
           int JSCheckSnMark, int JSCheckSimMark, int JSCheckBatMark, int JSCheckIccidMark, int JSCheckMacMark, int JSCheckEquipmentMark, int JSCheckVipMark, int JSCheckRfidMark, int PrintMode1, int PrintMode2, int JSCheckIMEI2Mark, int JSCheckInseIMEI2Mark,int RfidMark, string IMEI_Prefix, string IMEI_Suffix)
        {
            ...
            using (SqlConnection conn1 = new SqlConnection(conStr))
            {
                ...
                using (SqlCommand command = conn1.CreateCommand())
                {
                    command.CommandText = "SELECT " +
                        ...
                        "JSCheckRfidMark,   PrintMode1,       PrintMode2,           JSCheckIMEI2Mark,     JSCheckInseIMEI2Mark,  IMEI_Prefix,         IMEI_Suffix,       RfidMark     FROM dbo.Gps_ManuPrintRecordParam WHERE ZhiDan='" + ZhidanNum + "'";
                     ...
                     if (dr.Read())
                    {
                        ...
                        list.IMEI_Prefix = dr.IsDBNull(23) ? "" : dr.GetString(23);
                        list.IMEI_Suffix = dr.IsDBNull(24) ? "" : dr.GetString(24);
                        list.RfidMark = dr.IsDBNull(25) ? 0 : dr.GetInt32(25);
                    }
