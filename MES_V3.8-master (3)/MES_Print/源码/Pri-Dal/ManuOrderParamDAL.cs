...
namespace ManuOrder.Param.DAL
{
    class ManuOrderParamDAL
    {
       private static string conStr = ConfigurationManager.ConnectionStrings["conn1"].ConnectionString;
            ...
            using (SqlConnection conn1 = new SqlConnection(conStr))
            {
                ...
                using (SqlCommand command = conn1.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT       " +
                          "SoftModel,     SN1 ,       SN2 ,         SN3 ,        BoxNo1 ,          BoxNo2 ,           ProductDate,      Color ,       Weight ,       Qty ,         ProductNo ,   Version ,     IMEIStart ,   IMEIEnd ,    SIMStart , SIMEnd ,   BATStart ,  BATEnd ,    VIPStart ,   VIPEnd ," +
                          "IMEIRel ,      Remark1,    Remark5 ,     status ,     JST_template ,     CHT_template1 ,     CHT_template2 , BAT_prefix ,  BAT_digits ,   SIM_prefix ,  SIM_digits ,  VIP_prefix,   VIP_digits ,  ICCID_prefix ," +
                          "ICCID_digits , IMEIPrints, MAC_prefix ,  MAC_digits , Equipment_prefix , Equipment_digits ,  IMEI2Start ,    IMEI2End,     IMEI2Prints  , RFID_Start ,     RFID_End ,      RFID_prefix ,   RFID_digits,   Week  , IMEIMutiRange  FROM  dbo.Gps_ManuOrderParam WHERE ZhiDan='" + ZhidanNum + "' AND(Status = 0 or Status = 1 or Status = 2)";
                    SqlDataReader dr = command.ExecuteReader();
                    ...
                    if (dr.Read())
                    {
                        ...
                        list.Week = dr.IsDBNull(47) ? -1 : dr.GetInt32(47);

                        list.IMEIRangeNum = dr.IsDBNull(48) ? "" : dr.GetString(48);
                    }
        ...
        public Gps_ManuOrderParam selectManuOrderParamByzhidanDAL(string ZhidanNum,int NullInt)
        {
            ...
            using (SqlCommand command = conn1.CreateCommand())
            {
                ...
                command.CommandText = "SELECT "+
                    ...
                    " VIP_digits ,   ICCID_prefix ,  ICCID_digits , IMEIPrints ,   MAC_prefix , MAC_digits ,  Equipment_prefix , Equipment_digits , RFID_Start ,     RFID_End ,      RFID_prefix ,   RFID_digits  ,Week , IMEIMutiRange FROM dbo.Gps_ManuOrderParam WHERE ZhiDan='" + ZhidanNum + "'AND  (Status = 0 or Status =1 or Status = 2)";
                SqlDataReader dr = command.ExecuteReader();
                while (dr.Read())
                {
                    list.SoftModel = dr.GetString(0);
                    ...
                    list.Week = dr.IsDBNull(44) ? -1 : dr.GetInt32(44);
                    list.IMEIRangeNum = dr.IsDBNull(45) ? "" : dr.GetString(45);
        ...
        //根据制单号更新数据
        public int UpdateInlineByZhiDanDAL(string ZhiDanNum,string SN1, string SN2, string ProductData, string SIM1, string SIM2, string SIM_dig, string SIM_pre, string VIP1, string VIP2, string VIP_dig, string VIP_pre, string BAT1, string BAT2, string BAT_dig, string BAT_pre, string ICCID_dig, string ICCID_pre, string MAC_dig, string MAC_pre, string Equipment_dig, string Equipment_pre, string RFID_Num1,string RFID_Num2, string RFID_prefix,string RFID_digits,string Week)
        {
            SqlConnection conn1 = new SqlConnection(conStr);
            conn1.Open();
            using (SqlCommand command = conn1.CreateCommand())
            {
                command.CommandText = "UPDATE Gps_ManuOrderParam SET SN1='" + SN1 + "',SN2='" + SN2 + "',ProductDate='" + ProductData + "',SIMStart='" + SIM1 + "',SIMEnd='" + SIM2 + "',BATStart='" + BAT1 + "',BATEnd='" + BAT2 + "',VIPStart='" + VIP1 + "',VIPEnd='" + VIP2 + "',BAT_prefix='" + BAT_pre + "',BAT_digits='" + BAT_dig + "',SIM_prefix='" + SIM_pre + "',SIM_digits='" + SIM_dig + "',VIP_prefix='" + VIP_pre + "',VIP_digits='" + VIP_dig + "',ICCID_prefix='" + ICCID_pre + "',ICCID_digits='" + ICCID_dig + "',MAC_prefix='" + MAC_pre + "',MAC_digits='" + MAC_dig + "',Equipment_prefix='" + Equipment_pre + "',Equipment_digits='" + Equipment_dig + "',RFID_Start='" + RFID_Num1 + "',RFID_End='" + RFID_Num2 + "',RFID_prefix='" + RFID_prefix + "',RFID_digits='" + RFID_digits + "',Week ='" + Week + "' WHERE (ZhiDan='" + ZhiDanNum + "' AND Status=0)";
                return command.ExecuteNonQuery();
            }
        }
