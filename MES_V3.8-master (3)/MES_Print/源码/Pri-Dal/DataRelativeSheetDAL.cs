...
namespace DataRelative.Param.DAL
{
    ...
    class DataRelativeSheetDAL
    {
        ...
        //检查 IMEI 号是否绑定RFID，存在返回1，否则返回0
        public int CheckIMEIExitRFIDDAL(string IMEInumber)
        {
            using (SqlConnection conn1 = new SqlConnection(conStr))
            {
                conn1.Open();
                using (SqlCommand command = conn1.CreateCommand())
                {
                    command.CommandText = "SELECT IMEI13 FROM dbo.DataRelativeSheet WHERE IMEI1='" + IMEInumber + "'";
                    SqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
                    {
                        return 1;
